...
SEXP FitCallLocal_fun_Rcpp(SEXP &bppass_x_inn,SEXP &ChromInf_inn){
    ...
    // ---------------------------------------
    Rcpp::DataFrame bppass_x=Rcpp::as<Rcpp::DataFrame>(bppass_x_inn);
    Rcpp::DataFrame ChromInf=Rcpp::as<Rcpp::DataFrame>(ChromInf_inn);
    // ---------------------------------------
...
void EMstep2D_Rcpp(Rcpp::NumericVector &sdx_g,Rcpp::NumericVector &lambdax_g,
    ...
    for(int i=0;i<N;i++){
        ...
        for(int g=0;g<G;g++){//for each peak cluster only
            ...
            DensRespTot_g_Rcpp(g,i,sdx_g,lambdax_g,mx_g,sdy_g,
                               lambday_g,my_g,p_g,DensSum_i,DFit,Resp,Gupdate);
            // seet the ClassTot to zero if i==0, to initiate:
            if(i==0) ClassTot[g]=0;//initiate only once

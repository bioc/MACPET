...
exportPeaks.default = function(object,...) {
    stop(paste("No exportPeaks method for class ",class(object),sep=""))
}
...
exportPeaks.PSFit=function(object,file.out,savedir,threshold=NULL,...){
    ...
    skip="\n"
    writeLines(paste(Desc1,Desc2,skip,Chrom,Pets,Peak.Summit,Up.Summit,
                     Down.Summit,CIQ.Up.start,CIQ.Up.end,CIQ.Up.size,
                     CIQ.Down.start,CIQ.Down.end,CIQ.Down.size,CIQ.Peak.size,
                     lambdaUp,FoldEnrichUp,p.valueUp,
                     lambdaDown,FoldEnrichDown,p.valueDown,
                     p.value,FDRUp,FDRDown,FDR,skip,sep = "\n"),
               file.path(savedir,paste(file.out,".csv",sep="")))
    suppressWarnings(utils::write.table(x,quote=FALSE,
                                        file = file.path(savedir,paste(file.out,".csv",sep="")),
                                        sep = ";", col.names = TRUE,

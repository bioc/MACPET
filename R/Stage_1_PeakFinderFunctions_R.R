...
PeakFinder_fun = function(AnalysisDir, Selfobject, fileSelfFit, method, LogFile.dir) {
    ...
    #---------------Segment to regions:
    #--------------------------------------------
    SegmRes = FindRegions_fun(x = PETsData, ChromInf = ChromInf, LogFile.dir = LogFile.dir)
    ...
    #------------
    Ordermatch = match(RegionCounts$Chrom, S4Vectors::metadata(Selfobject)$Self_info$Chrom)
    S4Vectors::metadata(Selfobject)$Self_info$Region.counts[Ordermatch] = RegionCounts$Counts
    #--------------------------------------------
    #---------------Fit:
    #--------------------------------------------
    ...
    S4Vectors::metadata(Selfobject)$Classification.Info = GlobalPeakCallRes$Classification.Info
    #------Peak counts
    S4Vectors::metadata(Selfobject)$Self_info$Peak.counts = 0
    Match = match(GlobalPeakCallRes$Peak.counts$Chrom, S4Vectors::metadata(Selfobject)$Self_info$Chrom)
    S4Vectors::metadata(Selfobject)$Self_info$Peak.counts[Match] = GlobalPeakCallRes$Peak.counts$V1
    #--------------------------------------------
    #---------------Run inference:
    #--------------------------------------------
    ...
    InfRes = SignificanceCall_fun(Peaks.Info = Peaks.Info, PETsData = PETsData, ChromInf = ChromInf, 
        method = method, LogFile.dir = LogFile.dir)
    ...
    LogFile1 = paste("Total peak-calling time: ", Total.Time, " ", units(Total.Time), 
        "\n", sep = "")
    cat(LogFile1)
    write(LogFile1, file = LogFile.dir, append = TRUE)  #write in log file.
    # print its done:
    return("Binding Site Analysis is done!\n")
}
#-------------
#-------------
...
TakeFitAnalysisData_fun = function(x) {
    ...
    # return:
    #------------
    return(list(PETsData = x, ChromInf = ChromInf))
}
#-------------
#-------------
...
SignificanceCall_fun = function(Peaks.Info, PETsData, ChromInf, method, LogFile.dir) {
    ...
    # split by chromosome all the three inputs: then merge them by chromosome, pass a
    # three way list in c++

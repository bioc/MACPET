...
summary.PSelf=function(object,...){
    ...
    print(knitr::kable(PET.counts,row.names = FALSE,
...
summary.PSFit=function(object,threshold=NULL,...){
    ...
    if(!is.numeric(threshold)){
        colnames(PET.counts)=c("Chrom","Self-lig.","Regions",
                               "Peaks")
        cat(paste("|",paste(rep("-",6),collapse=""),
                  "Self-ligated PETs Summary",paste(rep("-",6),collapse=""),"|",
                  sep=""))
        Info=data.frame(Total.PETs=N,Regions=sum(PET.counts$`Regions`),
                        BS=sum(PET.counts$`Peaks`),
                        SLmean=SLmean,hg=hg,Organism=Organism,
                        MIN=paste(MinSize,"bp"),
                        MAX=paste(MaxSize,"bp"),class=class(object))
        colnames(Info)=c("Tot. Self-lig.","Regions","Peaks",
                         "Self-lig. mean size","Genome","Organism",
                         "Sortest Self-PET","Longest Self-PET","class")

        print(knitr::kable(PET.counts,row.names = FALSE,
                           align=c("c"),format="markdown",padding=1,
                           output=TRUE))
        print(knitr::kable(Info,row.names = FALSE,
                           align=c("c"),format="rst",padding=1,output=TRUE))
        ...
        PET.counts$Sign.BS.counts[match(Sign.peaks$Chrom,PET.counts$Chrom)]=
            Sign.peaks$V1
        colnames(PET.counts)=c("Chrom","Self-lig.","Regions",
                               "Peaks","Sign. Peaks")
        cat(paste("|",paste(rep("-",13),collapse=""),
                  "Self-ligated PETs Summary",paste(rep("-",13),collapse=""),
                  "|",sep=""))
        Info=data.frame(Total.PETs=N,Regions=sum(PET.counts$`Regions`),
                        BS=sum(PET.counts$`Peaks`),
                        Sign.BS=sum(PET.counts$`Sign. Peaks`),
                        SLmean=SLmean,hg=hg,Organism=Organism,
                        MIN=paste(MinSize,"bp"),
                        MAX=paste(MaxSize,"bp"),class=class(object))
        colnames(Info)=c("Tot. Self-lig.","Regions","Peaks",
                         "Tot. Sign. Peaks","Self-lig. mean size",
                         "Genome","Organism","Sortest Self-PET",
...
summary.PIntra=function(object,heatmap=FALSE,...){
    ...
    cat("|-------Summary-------|")
    Info=data.frame(Total.PETs=N,genome=genome,Organism=Organism,
                    class=class(object))
    colnames(Info)=c("Tot. Intra-chrom.","Genome","Organism","class")
    print(knitr::kable(PET.counts,row.names = FALSE,
                       align=c("l","c"),format="markdown",padding=1,
                       output=TRUE))
    print(knitr::kable(Info,row.names = FALSE,
                       align=c("c"),format="rst",padding=1,output=TRUE))
    if(heatmap){
        #check package:
        if(class(heatmap)!="logical"){
        ...
        RES=ggplot2::ggplot(data = plot.data, ggplot2::aes(x=Var1, y=Var2,
                                                           fill=value))+
            ggplot2::geom_tile(color="red")+
            ggplot2::ggtitle("HeatMap plot for Intra-chromosomal Counts")+
...
summary.PInter=function(object,heatmap=FALSE,...){
    ...
    Info=data.frame(Total.PETs=N,genome=genome,
                    Organism=Organism,class=class(object))
    colnames(Info)=c("Tot. Inter-chrom. PETs","Genome","Organism","class")
    cat("|--Inter-chrom. PETs--|\n")
    cat("|-------Summary-------|")
    print(knitr::kable(PET.countsPrint,row.names = FALSE,
                       align=c("l","c"),format="markdown",padding=1,
                       output=TRUE))
    print(knitr::kable(Info,row.names = FALSE,
                       align=c("c"),format="rst",padding=1,output=TRUE))
    if(heatmap){
        ...
        RES=ggplot2::ggplot(data = plot.data, ggplot2::aes(x=Var1, y=Var2,
                                                           fill=value)) +
            ggplot2::geom_tile(color="red")+
            ggplot2::ggtitle("HeatMap plot for Inter-chromosomal Counts")+
            ggplot2::xlab("Chromosome")+ggplot2::ylab("Chromosome")
        return(RES)
            }
    }

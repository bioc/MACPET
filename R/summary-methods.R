...
summary.PSelf = function(object, ...) {
    ...
    cat("|------Summary------|")
    Info = data.frame(Total.PETs = N, SLmean = SLmean, hg = hg,
                      MIN = paste(MinSize, "bp"), MAX = paste(MaxSize, "bp"),
                      class = class(object))
    colnames(Info) = c("Tot. Self-lig.", "Self-lig. mean size", "Genome",
                       "Sortest Self-PET",  "Longest Self-PET", "class")
    print(knitr::kable(PET.counts, row.names = FALSE, align = c("c"),
                       format = "markdown",  padding = 1, output = TRUE))
    print(knitr::kable(Info[, seq_len(3)], row.names = FALSE, align = c("c"),
                       format = "rst",  padding = 1, output = TRUE))
    print(knitr::kable(Info[, c(4, 5)], row.names = FALSE, align = c("c"),
                       format = "rst",  padding = 1, output = TRUE))
}
#' @rdname summary
#' @export
...
summary.PSFit = function(object, threshold = NULL, ...) {
    ...
    if (!is.numeric(threshold)) {
        ...
        cat("|", rep("-", 6), "Self-ligated PETs Summary", rep("-", 6), "|",
            sep = "")
        Info = data.frame(Total.PETs = N, Regions = sum(PET.counts$Regions),
                          BS = sum(PET.counts$Peaks), SLmean = SLmean, hg = hg,
                          MIN = paste(MinSize, "bp"),
                          MAX = paste(MaxSize, "bp"), class = class(object))
        colnames(Info) = c("Tot. Self-lig.", "Regions", "Peaks",
                           "Self-lig. mean size", "Genome", "Sortest Self-PET",
                           "Longest Self-PET", "class")
        print(knitr::kable(PET.counts, row.names = FALSE, align = c("c"),
                           format = "markdown", padding = 1, output = TRUE))
        print(knitr::kable(Info[, seq_len(4)], row.names = FALSE,
                           align = c("c"), format = "rst", padding = 1,
                           output = TRUE))
        print(knitr::kable(Info[, seq_len(4) + 4], row.names = FALSE,
                           align = c("c"), format = "rst", padding = 1,
                           output = TRUE))
    } else {
        ...
        PET.counts$Sign.BS.counts[match(Sign.peaks$Chrom,
                                        PET.counts$Chrom)] = Sign.peaks$V1
        colnames(PET.counts) = c("Chrom", "Self-lig.", "Regions", "Peaks",
                                 "Sign. Peaks")
        cat("|", rep("-", 13), "Self-ligated PETs Summary", rep("-", 13), "|",
            sep = "")
        Info = data.frame(Total.PETs = N, Regions = sum(PET.counts$Regions),
                          BS = sum(PET.counts$Peaks),
            Sign.BS = sum(PET.counts$`Sign. Peaks`), SLmean = SLmean, hg = hg,
            MIN = paste(MinSize, "bp"), MAX = paste(MaxSize, "bp"),
            class = class(object))
        colnames(Info) = c("Tot. Self-lig.", "Regions", "Peaks",
                           "Tot. Sign. Peaks", "Self-lig. mean size", "Genome",
                           "Sortest Self-PET", "Longest Self-PET", "class")
        print(knitr::kable(PET.counts, row.names = FALSE, align = c("c"),
                           format = "markdown", padding = 1, output = TRUE))
        print(knitr::kable(Info[, seq_len(4)], row.names = FALSE,
                           align = c("c"), format = "rst", padding = 1,
                           output = TRUE))
        print(knitr::kable(Info[, seq_len(5) + 4], row.names = FALSE,
                           align = c("c"), format = "rst", padding = 1,
                           output = TRUE))
    }
}
...
summary.PIntra = function(object, heatmap = FALSE, ...) {
    ...
    print(knitr::kable(PET.counts, row.names = FALSE, align = c("l", "c"),
                       format = "markdown", padding = 1, output = TRUE))
    print(knitr::kable(Info, row.names = FALSE, align = c("c"), format = "rst",
    ...
    if (heatmap) {
        ...
        RES = ggplot2::ggplot(data = plot.data, ggplot2::aes(x = Var1, y = Var2,
            fill = value)) + ggplot2::geom_tile(color = "red") +
            ggplot2::ggtitle("Heat-Map plot for Intra-chromosomal Counts") +
...
summary.PInter = function(object, heatmap = FALSE, ...) {
    ...
    PET.countsPrint = data.frame(Chrom = names(PET.countsPrint),
                                 Counts = PET.countsPrint)
    ...
    cat("|-------Summary-------|")
    print(knitr::kable(PET.countsPrint, row.names = FALSE, align = c("l", "c"),
                       format = "markdown", padding = 1, output = TRUE))
    print(knitr::kable(Info, row.names = FALSE, align = c("c"), format = "rst",
                       padding = 1, output = TRUE))
    if (heatmap) {
        ...
        RES = ggplot2::ggplot(data = plot.data, ggplot2::aes(x = Var1, y = Var2,
            fill = value)) + ggplot2::geom_tile(color = "red") +
            ggplot2::ggtitle("Heat-Map plot for Inter-chromosomal Counts") +
            ggplot2::xlab("Chromosome") + ggplot2::ylab("Chromosome")+
            ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5))
        return(RES)
    }

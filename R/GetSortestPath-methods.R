#' @title Find sortest path between pairs of peaks given a set of significant interactions.
...
#' To be published.
#'
#' @description \code{GetSortestPath} function subsets the significant interactions of a \code{\linkS4class{GenomeMap}} object
#' given a user specified FDR cut-off. Based on the leftover interactions, it creates a network and finds the length of the
#' sortest path between two user-specified peaks. Currently it only finds the sortest paths between intra-chromosomal peaks.
...
#' @param ... Further arguments to be passed to \code{GetSortestPath} (not used).
#'
...
#'
# default:
GetSortestPath = function(object, ...) {
    UseMethod("GetSortestPath", object = object)
}
#' @rdname GetSortestPath
#' @method GetSortestPath default
#' @export
GetSortestPath.default = function(object, ...) {
    stop("No GetSortestPath method for class ", class(object), ".", call. = FALSE)
}
#' @rdname GetSortestPath
#' @method GetSortestPath GenomeMap
...
#' and the second element named \code{ThreeDPathLength} for the 3D length of the sortest path
#' between \code{SummitFrom} and \code{SummitTo}.
#' @export
GetSortestPath.GenomeMap = function(object, threshold = NULL,ChrFrom, ChrTo, SummitFrom,SummitTo, ...) {
    # R check:
    seqnames1=NULL
    seqnames2=NULL
    #--------------------------
    ...
    #--------------------------
    SignInteractions = GetSignInteractions(object=object,
                             threshold = threshold, ReturnedAs='GInteractions')
    # make data frame, keep what you need:
    SignInteractions = as.data.frame(SignInteractions)
    SignInteractions = subset(SignInteractions, seqnames1==ChrFrom&seqnames2==ChrTo)
    Ninteractions=nrow(SignInteractions)
    if(Ninteractions==0){
        stop("No significant interactions were found between chromosomes: ",ChrFrom," and ",ChrTo," at the given threshold!",call. = FALSE)
    }
    SignInteractions=SignInteractions[,c("Anchor1Summit","Anchor2Summit")]
    #--------------------------
    # Make unique peaks data:
    #--------------------------
    Peaks=unique(c(SignInteractions$Anchor1Summit,SignInteractions$Anchor2Summit,SummitFrom,SummitTo))
    ...
    Network = Initiate_GenomeMap_fun_Rcpp(NPeaksInvolved, Peaks$PeakSummitIDS,
                    Peaks$PeakSummitIDS, Peaks$Summit, 0, FALSE)
    #--------------------------
    ...
    for(i in seq_len(Ninteractions)){
        ...
        if(length(Math1h)!=0) SignInteractions$Anchor1Summit[Math1h]=k
        Math2h=which(SignInteractions$Anchor2Summit==h)
        if(length(Math2h)!=0) SignInteractions$Anchor2Summit[Math2h]=k
        #--------------------------
        # update the network:
        #--------------------------

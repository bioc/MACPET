...
#-------------
#-------------
Stage_3_Main_fun = function(SA_prefix, S3_method, SA_LogFile.dir, S3_AnalysisDir,
    S3_Selfobject, S3_image) {
    ...
    Ordermatch = match(RegionCounts$Chrom,
                       S4Vectors::metadata(S3_Selfobject)$Self_info$Chrom)
    S4Vectors::metadata(S3_Selfobject)$Self_info$Region.counts[Ordermatch] =
        RegionCounts$Counts
    #--------------------------------------------
    #---------------Fit:
    #--------------------------------------------
    ...
    S4Vectors::metadata(S3_Selfobject)$Classification.Info =
    ...
    Match = match(GlobalPeakCallRes$Peak.counts$Chrom,
                  S4Vectors::metadata(S3_Selfobject)$Self_info$Chrom)
    S4Vectors::metadata(S3_Selfobject)$Self_info$Peak.counts[Match] =
    ...
    InfRes = SignificanceCall_fun(Peaks.Info = Peaks.Info, PETsData = PETsData,
                                  ChromInf = ChromInf, S3_method = S3_method,
                                  SA_LogFile.dir = SA_LogFile.dir)
    ...
    if (S3_image) {
        ...
        # image 1:
        Get_image_S3_P1_fun(S3_AnalysisDir = S3_AnalysisDir,
                            SA_prefix = SA_prefix, PeakInfo = PeakInfo)
        # image 2:
        Get_image_S3_P2_fun(S3_AnalysisDir = S3_AnalysisDir,
                            SA_prefix = SA_prefix, PeakInfo = PeakInfo)
        # image 3:
        Get_image_S3_P3_fun(S3_AnalysisDir = S3_AnalysisDir,
                            SA_prefix = SA_prefix, PeakInfo = PeakInfo)
        # image 4:
        Get_image_S3_P4_fun(S3_AnalysisDir = S3_AnalysisDir,
                            SA_prefix = SA_prefix, PeakInfo = PeakInfo)
    }
    #------------------------
    # save log:
    ...
    LogFile = paste("Stage 3 is done!\n")
    cat(LogFile)
    write(LogFile, file = SA_LogFile.dir, append = TRUE)
    LogFile = paste("Analysis results for stage 3 are in:\n",
                    S3_AnalysisDir, "\n")
    cat(LogFile)
    write(LogFile, file = SA_LogFile.dir, append = TRUE)
    # take time:
    ...
    LogFile = paste("Total stage 3 time: ", Total.Time, " ",
                    units(Total.Time), "\n", sep = "")
    cat(LogFile)
    write(LogFile, file = SA_LogFile.dir, append = TRUE)
}
...
TakeFitAnalysisData_fun = function(S3_Selfobject) {
    ...
    ChromInf = data.frame(Chrom = names(ChromInf),
                          size = as.numeric(ChromInf), stringsAsFactors = FALSE)
...
FindRegions_fun = function(PETsData, ChromInf, SA_LogFile.dir) {
    # rcheck:
    Chrom = Region = NULL
    #------------
    #------------
    ...
    SegmRes = BiocParallel::bplapply(X = bppass, FUN = function(y, ChromInf) {
        ...
        # Give the region ids:
        #------------
        y = RegionIds_fun(PETinf = y, ChromCur = ChromCur,
                          ChromInfCur = ChromInfCur)
        return(y)
    }, ChromInf = ChromInf)
    cat("Done\n")
    #------------
    ...
    LogFile = paste("Total Regions found: ", sum(RegionCounts$Counts), "\n")
    cat(LogFile)
    write(LogFile, file = SA_LogFile.dir, append = TRUE)  #write to log file
...
RegionIds_fun = function(PETinf, ChromCur, ChromInfCur) {
    ...
    #------------
    PETInt = intervals::Intervals(PETinf[, c("UTag", "DTag")],
                                  closed = c(TRUE, TRUE))
    #------------
    # Take clusters:
    #------------
...
FitCallGlobal_fun = function(SegPETS, ChromInf, SA_LogFile.dir) {
    #---R-check
    Region = Chrom = NULL
    #----
    ...
    GlobalFitRes = BiocParallel::bplapply(X = bppass,
                                          FUN = FitCallLocal_fun_Rcpp,
                                          ChromInf = ChromInf)
    cat("Done\n")
    #---------------------------
    #--------save Classification:
    ...
    LogFile = paste("Total", nrow(Peaks.Info),
                    "candidate peaks found in data.\n")
    cat(LogFile)
    write(LogFile, file = SA_LogFile.dir, append = TRUE)
    #---------------------------
    #--------Find counts:
    ...
    return(list(Peaks.Info = Peaks.Info,
...
SignificanceCall_fun = function(Peaks.Info, PETsData, ChromInf, S3_method,
    ...
    cat("Done\n")
    write("FDR adjusting p-values....Done\n", file = SA_LogFile.dir,
          append = TRUE)
    #------
    #---return:
    #------
...
Get_SignificanceCall_Data_fun = function(Peaks.Info, PETsData, ChromInf) {
    ...
    for (chri in seq_along(Peaks.Info_list)) {
        ...
        bppass[[chri]] = list(Peaks_Info_x = Peaks_Info_x,
                              PETsData_x = PETsData_x,
...
PoissonLocalInference_fun = function(bppass_x, windows) {
    ...
    }, lambdaUpW1 = lambdaUpW1, lambdaUpW2 = lambdaUpW2,
    lambdaBGC_Up = lambdaBGC_Up, lambdaDownW1 = lambdaDownW1,
    lambdaDownW2 = lambdaDownW2, lambdaBGC_Down = lambdaBGC_Down)
    ...
    pvalueUp = stats::ppois(q = Peaks_Info_x$Pets, lambda = OptimalLambda[, 1],
                            lower.tail = FALSE)
    # Downstream
...
Get_image_S3_P1_fun = function(S3_AnalysisDir, SA_prefix, PeakInfo) {
    ...
    FDR0.05=NULL
    # image dir:
    S3_P1_image_dir = file.path(S3_AnalysisDir,
                                paste(SA_prefix, "_stage_3_p1_image.jpg",
                                      sep = ""))
    # plot
    S3_image_p1 = ggplot2::ggplot(PeakInfo, ggplot2::aes(CIQ.Up.size, CIQ.Down.size,
        color = factor(FDR0.05))) + ggplot2::geom_point(size = 1) +
        ggplot2::theme_bw() +
        ...
        ggplot2::xlab("Upstream Peak Size") +
        ggplot2::ylab("Downstream Peak Size") +
        ggplot2::geom_abline(intercept = 0, slope = 1) +
        ggplot2::ggtitle("Upstream/Downstream peak sizes")
    # save:
...
Get_image_S3_P2_fun = function(S3_AnalysisDir, SA_prefix, PeakInfo) {
    ...
    FDR=NULL
    # image dir:
    S3_P2_image_dir = file.path(S3_AnalysisDir,
                                paste(SA_prefix, "_stage_3_p2_image.jpg",
                                      sep = ""))
    S3_image_p2 = ggplot2::ggplot(PeakInfo, ggplot2::aes(ID, sort(FDR, decreasing = FALSE))) +
        ggplot2::geom_line(size = 1) +
        ggplot2::geom_hline(yintercept = 0.05, color = "red", size = 1) +
        ...
        ggplot2::scale_y_continuous(expand = c(0, 0)) +
        ggplot2::scale_x_continuous(expand = c(0, 0)) +
        ggplot2::xlab("Sorted Binding Site IDs") +
        ggplot2::ylab("FDR") +
        ggplot2::ggtitle("FDR values for the binding sites")
    # save:
...
Get_image_S3_P3_fun = function(S3_AnalysisDir, SA_prefix, PeakInfo) {
    ...
    FDR0.05=NULL
    # image dir:
    S3_P3_image_dir = file.path(S3_AnalysisDir,
                                paste(SA_prefix, "_stage_3_p3_image.jpg",
                                      sep = ""))
    # plot
    S3_image_p3 = ggplot2::ggplot(PeakInfo, ggplot2::aes(CIQ.Peak.size, fill = factor(FDR0.05))) +
        ggplot2::geom_density(size = 1, alpha = 0.5) + ggplot2::theme_bw() +
        ggplot2::theme(axis.line = ggplot2::element_line(colour = "black"),
        panel.grid.major = ggplot2::element_blank(),
        panel.grid.minor = ggplot2::element_blank(),
        panel.background = ggplot2::element_blank()) +
        ...
        ggplot2::scale_y_continuous(expand = c(0, 0), labels = function(y) format(y,
            scientific = FALSE)) +
        ggplot2::scale_x_continuous(expand = c(0, 0)) +
        ggplot2::xlab("Binding Site Sizes") +
        ggplot2::ylab("Density") + ggplot2::geom_abline(intercept = 0, slope = 1) +
        ggplot2::ggtitle("Densities for the binding site sizes")
    # save:
...
Get_image_S3_P4_fun = function(S3_AnalysisDir, SA_prefix, PeakInfo) {
    ...
    FDR0.05=NULL
    # image dir:
    S3_P4_image_dir = file.path(S3_AnalysisDir,
                                paste(SA_prefix, "_stage_3_p4_image.jpg",
                                      sep = ""))
    # plot
    S3_image_p4 = ggplot2::ggplot(PeakInfo, ggplot2::aes(FDRUp, FDRDown, color = factor(FDR0.05))) +
        ...
        ggplot2::scale_y_continuous(expand = c(0, 0)) +
        ggplot2::scale_x_continuous(expand = c(0, 0)) +
        ggplot2::xlab("FDR for Upstream Binding Site Peak") +
        ggplot2::ylab("FDR for Downstream Binding Site Peak") +
        ggplot2::geom_abline(intercept = 0, slope = 1) +
        ggplot2::ggtitle("Upstream/Downstream peak FDR")
    # save:

...
PETClassification_fun=function(DataDir,DataFile,AnalysisDir,fileSelf,
                           fileIntra,fileInter,PopImage,
                           BlackList,LogFile.dir,Format,
    ...
    InArg=list(DataDir=DataDir,DataFile=DataFile,AnalysisDir=AnalysisDir,
             fileSelf=fileSelf,fileIntra=fileIntra,
             fileInter=fileInter,PopImage=PopImage,BlackList=BlackList,
             LogFile.dir=LogFile.dir,Format=Format,
             GenInfo=GenInfo,ChromLengths=ChromLengths)
    #--------------------------------------------
    #---------------Load input file:
    #--------------------------------------------
    ...
    if(length(pinterData)!=0){
        ...
        save(list=InArg$fileInter,
             file=file.path(InArg$AnalysisDir,InArg$fileInter))#save
    }else{#dont save any
    ...
    #---------------Find Intra PETs:
    #--------------------------------------------
    pintraData=FindIntra_fun(x=SIPairedData,SelfIndicator=SelfIndicator,
                             ArgFindIntra=InArg)
    ...
    if(length(pintraData)!=0){
        ...
        save(list=InArg$fileIntra,file=file.path(InArg$AnalysisDir,
                                                 InArg$fileIntra))#save
    }else{#dont save any
    ...
    #---------------Find Self PETs:
    #--------------------------------------------
    pselfData=FindSelf_fun(x=SIPairedData,SelfIndicator=SelfIndicator,
                           ArgFindSelf=InArg,PSelf.Convert=FALSE)
...
LoadData_fun=function(ArgLoadData){
    ...
    if(ArgLoadData$Format[length(ArgLoadData$Format)]=="sam"){
        ...
        Rsamtools::asBam(file=file,destination=destination,
                         overwrite=FALSE,indexDestination=TRUE)
        ArgLoadData$DataFile=paste(ArgLoadData$Format[1],".bam",sep="")
        #to convert to SAM format:
        # asSam(file= paste(DataDir,"/",DataFile,sep=""),
        # destination=paste(DataDir,"/","SAAAm",sep=""), overwrite=FALSE)
    ...
    if(!PairedEnd){
        stop("BAM file is not paired-end file!")
    }else{
...
GInteractionsCovnert_fun=function(ChIAPET,ArgPairedData){
    ...
    if(!is.null(ArgPairedData$LogFile.dir)){
        #then the function called by PETClassification
        #from tags
        ...
        #then the function called by ConvertToPSelf:
        #from tags
    ...
    if(!is.null(ArgPairedData$BlackList)){
        ...
        BlackListRM=BlackListCorrection_fun(Anchor1=Anchor1,Anchor2=Anchor2,
                                BlackList=ArgPairedData$BlackList,
                                LogFile.dir=ArgPairedData$LogFile.dir)
    }else{
        BlackListRM=c()
    }
    #------------
    #remove false mapped PETs, outside of genomic regions,
    #and unspecified strand anchors.
    #------------
    TrimmedRM=TrimAnchors_fn(Anchor1=Anchor1,Anchor2=Anchor2,
                           ArgPairedData=ArgPairedData)
    # total removals:
    ...
    if(length(Anchor1)==0){
        stop("All PETs ar removed from the data, exiting.")
    }
    #------------
    #make object:
    #------------
    ...
    DataSeqinfo=
        GenomeInfoDb::Seqinfo(
            seqnames=ChromInData,
            seqlengths=ArgPairedData$ChromLengths$size[Matchsl],
...
BlackListCorrection_fun=function(Anchor1,Anchor2,BlackList,LogFile.dir){
    ...
    #---------------
    BlackList=GenomicRanges::GRanges(seqnames=BlackList$Chrom,
                                     ranges=IRanges::IRanges(start=BlackList$Region.Start,
...
TrimAnchors_fn=function(Anchor1,Anchor2,ArgPairedData){
    cat("Checking if any PETs have to be trimmed...")
    #--------------------------------------------
    ...
    SeqPlace1=match(as.character(GenomicRanges::seqnames(Anchor1)),
                    ArgPairedData$ChromLengths$Chrom)
    EndAnchor1=which(GenomicRanges::end(Anchor1)>
                         ArgPairedData$ChromLengths$size[SeqPlace1])
    StrandAnchor1=which(as.character(GenomicRanges::strand(Anchor1))=="*")
    #------------
    #trim Anchor2:
    #------------
    StartAnchor2=which(GenomicRanges::start(Anchor2)<1)
    SeqPlace2=match(as.character(GenomicRanges::seqnames(Anchor2)),
                    ArgPairedData$ChromLengths$Chrom)
    EndAnchor2=which(GenomicRanges::end(Anchor2)>
                         ArgPairedData$ChromLengths$size[SeqPlace2])
    StrandAnchor2=which(as.character(GenomicRanges::strand(Anchor2))=="*")
    #------------
    #remove PETs:
    #------------
    RemAnchors=unique(c(StartAnchor1,EndAnchor1,StrandAnchor1,
                        StartAnchor2,EndAnchor2,StrandAnchor2))
    if(length(RemAnchors)!=0){
        LogFile1=paste(length(RemAnchors)," PETs were trimmed.\n")
        cat(LogFile1)
    }else{
        LogFile1="No PETs needed to be trimmed.\n"
        cat(LogFile1)
    }
    if(!is.null(ArgPairedData$LogFile.dir)){
        write(LogFile1,file=ArgPairedData$LogFile.dir,append=TRUE)#write in log file.
...
FindInter_fun=function(x,ArgFindInter){
    ...
    if(length(interpets)!=0){
        ...
        #find counts:
        #------------
        Inter.Counts=data.frame(seqnames1=GenomeInfoDb::seqnames(Anchor1),
                                seqnames2=GenomeInfoDb::seqnames(Anchor2),
                                stringsAsFactors=FALSE)
        Inter.Counts=table(Inter.Counts)
        Inter.Counts=as.data.frame.matrix(Inter.Counts,stringsAsFactors=FALSE)
        #order:
        Order.col=match(gtools::mixedsort(colnames(Inter.Counts)),
                        colnames(Inter.Counts))
        Inter.Counts=Inter.Counts[,Order.col]
        Order.row=match(gtools::mixedsort(rownames(Inter.Counts)),
                        rownames(Inter.Counts))
...
FindSelfIntra_fun=function(x,ArgFindSelfIntra){
    ...
    slope=(point2$Freq-point1$Freq)/#slope
        (point2$logSize-point1$logSize)
    intercept=point1$Freq-slope*point1$logSize
    #-----find distance to line
    SpanDF$PointDist=abs(-slope*SpanDF$logSize+1*SpanDF$Freq-intercept)/
        sqrt(slope^2+1)
    ...
    LogFile1=paste("Self-ligated cut-offs at: ",SelfBorder$MIN,"bp and ",
                   SelfBorder$MAX,"bp\n")
    cat(LogFile1)
    ...
    if(ArgFindSelfIntra$PopImage){
        # Plot the elbow:
        Self_ligated_borders_plot=
            ggplot2::ggplot(SpanDF,ggplot2::aes(x=Size,y=Freq))+
            # add histogram:
            ggplot2::geom_rect(ggplot2::aes(xmin=SpanDF$Size-49,
                                            xmax=SpanDF$Size+49,
                                            ymin=0,ymax=SpanDF$Freq),
                               size=0.3,fill="grey69",color="black")+
            # add line:
            ggplot2::geom_line(color="blue",size=0.6)+
            # add cut-off line:
            ggplot2::geom_vline(xintercept=ElbowPoint$Size,
                                linetype="dashed",color="red",size=0.6)+
            # change scale:
            ggplot2::scale_x_log10(labels=function(co) round(log10(co)))+
            # titles
            ggplot2::ggtitle(paste("Elbow-point cut-off for Self/Intra Pets at: "
                                   ,SelfBorder$MAX," bp"))+
            ggplot2::xlab("Sorted log-sizes of PET")+
            ...
            ggplot2::theme(axis.text=
                               #numbers axis
                               ggplot2::element_text(size=15,color="black"),
                           axis.title=
                               #axis text
                               ggplot2::element_text(size=18,color="black"),
                           plot.title=
                               #title
                               ggplot2::element_text(size=20,color="black"))+
            ggplot2::theme(plot.title=ggplot2::element_text(hjust=0.5))

        ggplot2::ggsave(plot=Self_ligated_borders_plot,
                        file=file.path(ArgFindSelfIntra$AnalysisDir,
...
FindIntra_fun=function(x,SelfIndicator,ArgFindIntra){
    cat(paste("Separating Intra-chromosomal data...\n"))
    #------------
    #keep intra:
    #------------
    ...
    if(length(intrapets)!=0){
        ...
        Intra.counts=data.frame(Chrom=ChrNames,Counts=CountValues,
                                stringsAsFactors=FALSE)
        #sort:
...
FindSelf_fun=function(x,SelfIndicator,ArgFindSelf,PSelf.Convert){
    cat(paste("Separating Self-ligated data...\n"))
    #------------
    #keep self:
    #------------
    ...
    #Take info:
    #------------
    Self_info=table(GenomicRanges::seqnames(
        InteractionSet::anchors(selfpets,type="first")))
    Self_info=data.frame(Chrom=as.character(names(Self_info)),
                         PET.counts=as.numeric(Self_info))

...
InputCheckPeakCallerUlt = function(InArg) {
    ...
    DefaultInputs = c("DataDir", "DataFile", "AnalysisDir", "GenomePkg", "PopImage", 
        "fileSelf", "fileIntra", "fileInter", "BlackList", "fileSelfFit", "method", 
        "Stages")
    ...
    if (!any(InArg$Stages %in% c(0:1))) {
        stop("Stages argument should take one of the following elements: 0, 1, c(0,1).", 
            call. = FALSE)
    } else {
    ...
    if (c(0) %in% InArg$Stages) {
        if (!is.logical(InArg$PopImage)) {
            stop("PopImage has to be logical!", call. = FALSE)
        } else {
            if (!requireNamespace("ggplot2", quietly = TRUE)) {
                stop("ggplot2 needed for this function to work if PopImage==T. Please install it.", 
                  call. = FALSE)
            }
        }
    }
    ...
    if (c(0) %in% InArg$Stages) {
        ...
        if (is.character(InArg$DataFile) && length(InArg$DataFile) == 1) {
            if (!dir.exists(InArg$DataDir)) {
                ...
                if (length(Format) < 2) {
                  ...
                  stop("DataFile is neither a BAM nor a SAM format!", call. = FALSE)
                } else {
                  InArg$Format = Format
                ...
                stop("DataFile does not exist in the DataDir directory!", call. = FALSE)
            }
        } else if (!is(InArg$DataFile, "GAlignmentPairs")) {
            stop("DataFile has to be a BAM/SAM file name, or an object of class GAlignmentPairs.", 
                call. = FALSE)
        } else {
    ...
    #--------------------------
    if (c(0) %in% InArg$Stages) {
        if (!is.character(InArg$GenomePkg) || length(InArg$GenomePkg) != 1) {
            stop("GenomePkg has to be of character class!", call. = FALSE)
        } else if (!InArg$GenomePkg %in% BSgenome::available.genomes()) {
            stop(InArg$GenomePkg, " is not a part of the available.genomes! See ??BSgenome::available.genomes", 
                call. = FALSE)
        } else if (!InArg$GenomePkg %in% BSgenome::installed.genomes()) {
            stop(InArg$GenomePkg, " is not installed! See ??BSgenome::installed.genomes", 
                call. = FALSE)
        } else {
    ...
    if (c(0) %in% InArg$Stages) {
        if (is.data.frame(InArg$BlackList)) {
        ...
        } else if (is.character(InArg$BlackList) && length(InArg$BlackList) == 1) {
            # probably correct inputs.
            genome.given = InArg$BlackList  #the given genome
            ...
            if (!genome.given %in% names(sysdata)) {
                ...
                InArg$BlackList = sysdata[[genome.given]]
            }
        } else {
            # wrong inputs
    ...
    if (c(1) %in% InArg$Stages) {
        if (!InArg$method %in% stats::p.adjust.methods) {
            stop("method value is wrong! See ??stats::p.adjust.methods.", call. = FALSE)
        }
    }
    #--------------------------
    ...
    #--------------------------
    if (c(0) %in% InArg$Stages) {
        if (!is.character(InArg$fileSelf) | !is.character(InArg$fileIntra) | !is.character(InArg$fileInter)) {
    ...
    if (c(1) %in% InArg$Stages) {
        ...
        # if stage 0 or ALL is NOT in the stages, then the fileSelf has to be loaded and
        # it has to be PSelf class!
        if (!c(0) %in% InArg$Stages) {
            # means only stage 1 is to be run.  check if file name correct:

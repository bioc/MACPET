...
Model based analysis for ChIA-PET data (\Rpackage{MACPET}) is a method/pipeline 
for finding binding sites (peak calling) using ChIA-PET data (or paired-end data in general).
\Rpackage{MACPET} uses information from both paired-ends of ChIA-PET data and searches for two-dimensional 
...
The output from \Rpackage{MACPET} can be used for interaction analysis using either MANGO or MICC. However there are plans to extend the package and include interaction analysis in the future. Note that in the case of using the output from \Rpackage{MACPET} in MANGO or MICC for interaction analysis, the user should use the self-ligated cut-off found by \Rpackage{MACPET}, and not the one found in MANGO or MICC. Both of those algorithms allow the user to specify the self-ligated cut-off.

The main \Rpackage{MACPET}
...
by \Rpackage{MACPET} for finding significant peaks. Most of the \Rpackage{MACPET} functions require a user-specified path where the output of 
the algorithm is to be saved.

\Rpackage{MACPET} can analyse ChIA-PET data for both BAM and SAM formats and is 
...
before running a \Rpackage{MACPET} function. However most of the MACPET algorithms 
are implemented in C++ and are therefore very fast. 


Before starting with examples of how to use \Rpackage{MACPET}, create a 
...
<!-- Classes -->
\section{\Rpackage{MACPET} Classes}
\label{sec:classes}

\Rpackage{MACPET} provides four different classes which all inherit from the 
\Rclass{GInteractions} class in the \Biocpkg{InteractionSet} package:

* \textcolor{LimeGreen}{\Rclass{PSelf}} class contains information 
about the self-ligated PETs in the data.
...
<!--  methods -->
\section{\Rpackage{MACPET} Methods}

This section describes methods associated with the classes in the \Rpackage{MACPET} package.

<!-- summary -->
\subsection{summary-method}
All \Rpackage{MACPET} classes are associated with a summary method which sums
up the information stored in each class:

\subsubsection{\textcolor{LimeGreen}{\Rclass{PSelf}} Class}

\Rfunction{summary} for \textcolor{LimeGreen}{\Rclass{PSelf}} class:
```{r}
class(pselfData)
summary(pselfData)
```
...
\Rfunction{summary} for \textcolor{LimeGreen}{\Rclass{PSFit}} class:
```{r}
class(psfitData)
summary(psfitData)
```
...
\Rfunction{summary} for \textcolor{LimeGreen}{\Rclass{PIntra}} class:
```{r}
class(pintraData)
...
\Rfunction{summary} for \textcolor{LimeGreen}{\Rclass{PInter}} class where the user can 
choose to plot a heat-map for the total PETs between the chromosomes:
```{r}
class(pinterData)
requireNamespace("ggplot2")
requireNamespace("reshape2")
...
All \Rpackage{MACPET} classes are associated with a plot method which can be 
...
\subsubsection{\textcolor{LimeGreen}{\Rclass{PSelf}} Class}
\Rfunction{plot} for \textcolor{LimeGreen}{\Rclass{PSelf}} Class:
```{r}
requireNamespace("ggplot2")
class(pselfData)
# PET counts plot
...
\subsubsection{\textcolor{LimeGreen}{\Rclass{PSFit}} Class}
\Rfunction{plot} for \textcolor{LimeGreen}{\Rclass{PSFit}} Class:
```{r}
class(psfitData)
#binding site couts:
...
\subsubsection{\textcolor{LimeGreen}{\Rclass{PIntra}} Class}
\Rfunction{plot} for \textcolor{LimeGreen}{\Rclass{PIntra}} Class:
```{r}
class(pintraData)
#plot counts:
plot(pintraData)
...
\textcolor{LimeGreen}{\Rclass{PSFit}}
class has also a method which converts the peaks found by the peak-calling algorithm
into \Rclass{GRanges} objects with start and end coordinates the peak's confidence interval (CIQ.Up.start,CIQ.Down.end).
...
peak-calling algorithm. This method could be used in case the user already has the self-ligated PETs
...
classes of the \Rpackage{MACPET} package for printing and/or saving statistics
